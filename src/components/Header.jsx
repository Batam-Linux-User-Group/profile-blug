      ...
      <nav className="lg:max-w-screen-lg mx-auto flex flex-wrap items-center justify-between">
        ...
        <div className="flex-1 flex justify-end text-secondary-dark font-bold group">
          <a
            ...
            className="bg-primary border-4 border-secondary-dark px-4 py-2 rounded-full group-hover:scale-105 transition-all delay-300 text-sm"
          >
            Daftar OSC 2025
          </a>
        </div>
...
const NavItems = ({ label, link, isActive }) => {
  return (
    <li className="text-secondary-dark relative group flex justify-center">
      <Link to={link} className={`${isActive ? 'font-bold' : 'font-normal'}`}>
        {label}
      </Link>
      <div
        className={`absolute -bottom-1 bg-primary transition-all delay-150 h-1 ${
